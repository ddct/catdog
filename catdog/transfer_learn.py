...
# Cat and dogs images can be found in the file train.zip found at
# https://www.kaggle.com/c/dogs-vs-cats-redux-kernels-edition/data.
# create a folder named data.  Under that folder create the subfolders "train" and "validate"
# Copy 1000 "cats" files to the data/train/cats folder, 1000 "dogs" files to the data/train/dogs folder.
# Copy 400 different "cats" files to the data/validate/cats folder,
# 400 different "dogs" files to the data/validate/dogs folder.
...
#   Define image generators that will variations of image with
#   the image r/otated slightly, shifted up, down, left, or right,
#   sheared, zoomed in, or flipped horizontally on the vertical axis
#   (ie. person looking to the left ends up looking to the right)
...
InceptionV3_base_model = InceptionV3(include_top=False)  # include_top=False excludes final FC layer
...
z = Dense(Number_FC_Neurons, activation='relu')(y)        # new FC layer, random init
predictions = Dense(num_classes, activation='softmax')(z)  # new softmax layer
...
print(str(num_train_samples) + "\n" + str(validation_generator) + "\n" + str(num_validate_samples) + "\n"
      + str(num_epoch) + "\n" + str(train_generator.n))
...
model.save('./models/inceptionv3-transfer-learning.model')
...
# By using generators we can ask continue to request sample images
# and the generators will pull images from the training or validation
...
model.save('./models/inceptionv3-fine-tune.model')
